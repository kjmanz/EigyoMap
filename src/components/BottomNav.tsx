...
const TABS: { to: string; nav: AppHeaderActiveNav; label: string; Icon: () => JSX.Element }[] = [
  ...
  { to: "/settings", nav: "settings", label: "設定", Icon: SettingsIcon },
];

export function BottomNav({ active }: Props) {
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-[2000] border-t border-gray-200 bg-white"
      style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
    >
      <div className="flex h-14 items-stretch">
        {TABS.map(({ to, nav, label, Icon }) => {
